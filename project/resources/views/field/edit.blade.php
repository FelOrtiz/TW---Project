	...
	<section class="content">
		...
		<div class="box box-success">
			<div class="box-header with-border">
				<h3 class="box-title">Editar Cancha</h3>
			</div>
			<form method="POST" role="form" action="/field/update/{{ $field->id }}">
				{{ csrf_field() }}
				<div class="box-body">
					<div class="row">
						<div class="col-md-12">
							<div class="form-group has-feedback {{ $errors->has('name') ? 'has-error': '' }}">
								<label>Nombre</label>
								<input type="text" class="form-control" placeholder="EJ: Cancha 1" name="name" value="{{ $field->name }}">
								@if ($errors->has('name'))
							...
							<div class="form-group has-feedback {{ $errors->has('enclosure_id') ? 'has-error': '' }}">
								...
								<select class="form-control" name="enclosure_id">
									...
									@if($enclosure->id == $field->enclosure_id)
											<option value="{{ $enclosure->id }}" selected="">{{ $enclosure->name }}</option>
										@else
							...
							<div class="form-group has-feedback {{ $errors->has('capacity') ? 'has-error': '' }}">
								...
								<input type="integer" class="form-control" name="capacity" value="{{ $field->capacity }}">
							...
							<div class="form-group has-feedback {{ $errors->has('type_id') ? 'has-error': '' }}">
								...
								<select class="form-control" name="type_id">
									...
									@if($fieldType->id == $field->type_id)
											<option value="{{ $fieldType->id }}" selected="">{{ $fieldType->name }}</option>
										@else
							...
							<div class="form-group has-feedback {{ $errors->has('init_hour') ? 'has-error': '' }}">
								...
								<input type="timestamp" class="form-control" name="init_hour" value="{{ $field->init_hour}}">
							...
							<div class="form-group has-feedback {{ $errors->has('end_hour') ? 'has-error': '' }}">
								...
								<input type="timestamp" class="form-control" name="end_hour" value="{{ $field->end_hour}}">
				...
				<div class="box-footer">
					...
					<button type="submit" class="btn btn-success btn-flat pull-right">Editar Cancha </button>
				</div>
			</form>
		</div>
	</section>
